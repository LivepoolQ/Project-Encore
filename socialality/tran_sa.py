...
@LastEditTime: 2026-07-27 16:17:48
...
class TransformerSAModel(Model):
    ...
    def forward(self, inputs, training=None, mask=None, *args, **kwargs):
        ...
        f_tran_pool = f_tran.mean(dim=1)

        # Prediction
        for _ in range(repeats):
            z = torch.normal(mean=0, std=1, size=(f_tran.shape[0], self.r.generation_num, self.d_id)).to(f_tran.device)
            f_z = self.ie(z)
            # (batch, Kc, out_uni * 4)
            f_tran_expand = f_tran_pool.unsqueeze(1).repeat(1, self.r.generation_num, 1)
            f_multi = torch.cat([f_tran_expand, f_z], dim=-1)
...
class TransformerSA(Structure):
    MODEL_TYPE = TransformerSAModel
